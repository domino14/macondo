...
func TestHashAfterMakingPlay(t *testing.T) {
	...
	is.NoErr(err)
	alph := tilemapping.EnglishAlphabet()
	h := z.Hash(g.Board().GetSquares(), tilemapping.RackFromString("ADENOOO", alph), tilemapping.RackFromString("AHIILMM", alph), false, 0, 0)

	m1 := move.NewScoringMoveSimple(8, "15J", "END", "AOOO", alph)
	h1 := z.AddMove(h, m1, true, 0, 0, 8, 0)

	// Actually play the move on the board.
	err = g.PlayMove(m1, false, 0)
	is.NoErr(err)

	h2 := z.Hash(g.Board().GetSquares(), tilemapping.RackFromString("AOOO", alph), tilemapping.RackFromString("AHIILMM", alph), true, 0, 8)
	is.Equal(h1, h2)
}
...
func TestHashAfterPassing(t *testing.T) {
	...
	h2 := z.Hash(g.Board().GetSquares(), tilemapping.RackFromString("ADENOOO", alph), tilemapping.RackFromString("AHIILMM", alph), true, 1, 0)
...
func TestHashAfterMakingAnotherPlay(t *testing.T) {
	...
	is.NoErr(err)
	alph := tilemapping.EnglishAlphabet()
	h := z.Hash(g.Board().GetSquares(), tilemapping.RackFromString("AOOO", alph), tilemapping.RackFromString("HI", alph), false, 0, 0)
	fmt.Println(g.ToDisplayText())
	m1 := move.NewScoringMoveSimple(11, "13G", ".O.O", "AO", alph)
	h1 := z.AddMove(h, m1, true, 0, 0, 11, 0)
	...
	h2 := z.Hash(g.Board().GetSquares(), tilemapping.RackFromString("AO", alph), tilemapping.RackFromString("HI", alph), true, 0, 11)
	is.Equal(h1, h2)
}
