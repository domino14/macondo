...
//
// Populated during exchange generation (which already enumerates all
// rack subsets), so there is no additional KLV lookup cost.
//
// Ported from magpie's leave_map.h.
...
// storeCurrentLeave computes and stores the leave value for the current
// rack state (identified by currentIndex).
func (lm *leaveMap) storeCurrentLeave(klv *equity.KLV, rack *tilemapping.Rack) {
	...
	lm.values[lm.currentIndex] = klv.LeaveValue(leave[:n])
}
...
// populateLeaveMap enumerates all rack subsets and stores leave values.
// Used when exchanges aren't generated but the leave map is still needed.
...
func (gen *GordonGenerator) populateLeaveMapRecursive(rack *tilemapping.Rack, ml tilemapping.MachineLetter) {
	...
	if int(ml) == len(rack.LetArr) {
		...
		// Track best leave per size for shadow bounds
		numOnRack := int(rack.NumTiles())
		val := gen.leavemap.values[gen.leavemap.currentIndex]
