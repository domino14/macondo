...
def generate_leaves_to_from_length(
    tile_bag, start_length, end_length, allow_duplicates
):
    leaves = []
...
def smoothable(key, dp):
    return (len(key) == 5 and dp[COUNT_INDEX] < 828) or (
        len(key) == 6 and dp[COUNT_INDEX] < 234
    )
...
def get_neighboring_leaves(leave):
    ...
    for i in range(len(leave)):
        ...
        for letter in ALPHABETICAL_KEY[1:]:
            neighbors.append(
                "".join(sorted(leave[:i] + letter + leave[i + 1 :], key=SORT_FUNC))
            )
    return neighbors
    ...
    with open(move_logfile, "r") as f:
        ...
        for i, row in enumerate(moveReader):
            ...
            if int(tiles_remaining) >= TILE_LIMIT:
                ...
                row_count += 1
                subleaves = generate_leaves_to_from_length(
                    row_rack, 1, MAXIMUM_SUPERLEAVE_LENGTH, False
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate superleaves with a single command. Must be executed from the superleaves directory."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=200000,
        help="number of autoplay games per iteration",
    )
    parser.add_argument(
        "-l",
        "--lexicon",
        type=str,
        default="CSW19",
        help="lexicon used to generate superleaves",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="autogenerate_{}".format(CURRENT_DATE),
    ...
    for x in range(iterations):
        ...
        # Autoplay
        autoplay_cmd = "../../bin/shell autoplay -numgames {} -lexicon {} -logfile {} -block true".format(
            games, lexicon, autoplay_logfile
        )
        ...
        print(
            "leaves CSV file created. Please run wolges program to convert to klv2 format for autoplay."
        )
