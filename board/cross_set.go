...
func (b *GameBoard) updateCrossSetsForMove(m *move.Move, gd gaddag.SimpleGaddag,
	...
	calcForAcross := func(rowStart uint8, colStart uint8, csd BoardDirection) {
		for row := rowStart; row < uint8(len(m.Tiles()))+rowStart; row++ {
	...
	calcForSelf := func(rowStart uint8, colStart uint8, csd BoardDirection) {
